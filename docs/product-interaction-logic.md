...
6. **设置**：专注时长、连续专注上限、短休、长休、长休周期，以及四类独立提醒的开关和间隔。
...
1. **压力动画与 40 分钟爆炸并非完全同步**：默认压力每分钟只增加 1 点，角色要到 55 点才进入变红动画，但连续专注 40 分钟已经会强制爆炸。全新一天从 0 压力开始时，可能尚未进入明显压力形态就触发爆炸。后续应统一“视觉压力曲线”和“连续专注上限”。
2. **两套提醒机制并存**：每个番茄休息固定有四项轮播，同时还保留按 20/45/50/120 分钟触发的独立提醒。如果产品最终只希望在番茄休息中提醒，应关闭或移除独立调度器。
3. **部分短暂反馈没有专用动效**：运行时会产生 `happy` 和短休完成后的 `rest` 状态，但当前播放器没有给它们配置独立视频，会回退为普通静态待机图。
