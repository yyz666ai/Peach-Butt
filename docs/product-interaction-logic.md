...
- 默认气泡文案是“我会安静陪你”。
- **深夜陪伴模式（2026-08-30 新增）**：23:00–6:00 的待机不再播 idle，改为 `sleep.webm` 打瞌睡，气泡「早点睡啦，我陪你，但不鼓励熬夜」；若最近 5 分钟内还有用户交互（说明人还醒着），改为揉眼劝睡文案「这么晚还在忙？揉揉眼睛，早点睡吧」（复用 eye-strain 素材）。深夜时段自动抑制无聊求关注和随机小动作插播，专注/休息/提醒等业务状态不受影响。
...
**连击害羞（2026-08-30 新增）**：普通待机下 10 秒内连续点击 3 次以上，从「嘿嘿，被你发现啦」的开心反馈升级为 `shy.webm` 害羞动画（捂脸、从指缝偷看），气泡「别、别一直戳啦…」；停顿超过 10 秒后连击清零。

### 3.5 陪伴里程碑（2026-08-30 新增）

- 有任意活动记录（打卡、专注、休息、点击）的天数即「互相陪伴天数」；启动当天隐式计入。
- 恰好在第 7 / 30 / 100（及之后每满 100）天的当天首次启动时，改播 `dance.webm` 庆祝舞蹈 + 彩带，气泡「我们互相陪伴 N 天啦！以后也要一起哦」，替代当天的每日问候，同时触发一次窗口内撒花。
- 非里程碑日不受影响。素材由 MiniMax-H3 图生视频生成。

### 3.5b 成长等级（2026-08-30 新增）

- **累计能量** = 历史每天健康分（scoreEnd）的总和，随打卡/休息实时增长；取**高水位**记录，爆炸扣分不会导致降级。
- 等级映射：桃苗(0+)→小桃(200+)→圆桃(500+)→蜜桃(1200+)→仙桃(3000+)。
- 跨过门槛升级时，播一次 `transform.webm` 变身动画，气泡「我长大啦！现在是XX了！」，同时触发一次窗口内撒花。
- 每升一级解锁一句新的待机口头禅加入气泡池（如小桃「长出小桃子啦，一起加油」、仙桃「仙桃状态全开，放心交给我」），按分钟轮换。
- 当前等级随快照下发（`AppSnapshot.growth`，含 level / name / energy / days），供界面展示与气泡池使用。首次启动以当前累计能量为基线，不会误播升级动画。

### 3.5c 撒花与小屋徽章（2026-08-30 新增）

- **撒花**：里程碑 dance 或升级 transform 出现时，桃屁屁窗口内播放约 4.8 秒纯 CSS 粒子撒花（48 片彩色纸屑，无新素材）；同一事件只触发一次（`celebrationKey` 去重）。
- **小屋徽章**：小屋 Dashboard 成长卡片底部固定徽章条，共 7 枚——成长线「小桃/圆桃/蜜桃/仙桃」（按等级解锁）+ 陪伴线「一周之约 7 天 / 满月陪伴 30 天 / 百日相守 100 天」（按陪伴天数解锁）。未解锁置灰，右下角显示「已收集 N/7」。纯逻辑在 `src/renderer/src/components/badges.ts`。
...
瘪气期间播放 `deflated.webm` 皱巴巴缓慢呼吸循环动画（2026-08-30 起替换原静态图），并随恢复进度从 0.72 倍缓慢放大。
...
6. 条件满足后恢复值直接回到 100，并播放变身动画，气泡「谢谢你等我回来～」（2026-08-30 起替代原「休息够啦，恢复活力！」）。
7. 变身完成后解除专注锁，回到正常待机。
8. **和好奖励（2026-08-30 新增）**：恢复成功当天，第一次健康打卡（喝水/活动/上厕所/护眼/休息轮打卡均算）额外 +5 分，健康分事件记录 reason 为 `reconciliation`；当天只发一次，标记跨重启持久化——减少爆炸的纯惩罚感。
...
| `eye-strain` / `eye-rest` | `eye-strain.webm` 0–5s | 单次 | 护眼任务或逾期提醒；深夜用户仍活跃时 | 停末帧或切换任务 |
| `sleep` | `sleep.webm` 0.05–4.9s | 循环 | 长休四项全部完成；深夜（23:00–6:00）待机 | 长休结束；白天到来或交互后回待机 |
...
10. 手动打招呼
11. 短暂点击反馈
12. 普通待机（23:00–6:00 时改为深夜打瞌睡/揉眼形态）
...
另有一次性加分项：爆炸恢复当天的第一次打卡额外 **+5**（和好奖励，reason 为 `reconciliation`，每天最多一次）；里程碑/升级等特殊场景通过 `bonusScore` 通用加分接口发放。
...
- 瘪气锁定与恢复计时
- 当日分数、压力、计数和连续专注进度
- 未发放的和好奖励标记（`session.reconciliationDay`）
- 成长等级与累计能量高水位（`settings` 中的 `growthEnergy`）
...
| 健康分、压力、爆炸、通用加分 | `src/core/health-engine.ts` |
| 每轮四项任务 | `src/core/rest-session.ts` |
| 独立提醒调度 | `src/core/reminders.ts` |
| 视觉状态选择 | `src/core/pet-visual-state.ts` |
| 深夜模式 / 成长等级 / 和好奖励调度 | `src/main/runtime.ts`（isLateNight / syncGrowthEnergy / applyReconciliationBonus） |
